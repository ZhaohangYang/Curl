...
class HuobanWechat {
    ...
    public static function openid($appid, $secret, $code) {        
        return CurlHttp::get('https://api.weixin.qq.com/sns/oauth2/access_token', array('appid' => $appid, 'secret'=>$secret, 'code'=>$code, 'grant_type'=>'authorization_code'), array('pass_version'=>true));              
    }
    ...
    public static function accessToken($appid, $secret) {
        ...
        if(empty($token_time[0]) || (time() - $token_time[1] >= 5400)) {
            ...
            $res = CurlHttp::get('https://api.weixin.qq.com/cgi-bin/token', array('appid' => $appid, 'secret'=>$secret, 'grant_type'=>'client_credential'), array('pass_version'=>true));            
    ...
    public static function jsapiTicket($access_token) {
        ...
        if(empty($ticket_time[0]) || (time() - $ticket_time[1] >= 5400)) {
            ...
            $res = CurlHttp::get('https://api.weixin.qq.com/cgi-bin/ticket/getticket', array('access_token' => $access_token, 'type'=>'jsapi'), array('pass_version'=>true));            
            $str = $res['ticket'].':'.time();
